  ...
  (SELECT M.NUMTRANSITEM AS NUMTRANSITEM_ENTTV9,
    ...
    WHERE M.CODFISCAL IN (SELECT CODFISCAL FROM vENT_DEMONSTRACAO)
     ...
     (SELECT M.NUMTRANSITEM,
             ...
             END) PRAZO,
             NVL(M.CODUSUR,0) CODUSUR,
             (CASE WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vSAID_DEVOLUCAO) THEN S.CODCLI ELSE 0 END) CODFORNEC,
             (CASE WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vSAID_DEVOLUCAO) THEN 0 ELSE S.CODCLI END) CODCLI,
             M.NUMNOTA,
             M.DTMOV DATA,
             M.CODPROD,
             M.QTCONT QT,
             ...
             ROUND(NVL(M.CUSTOREP, 0),4) CUSTOREPOSICAO,
             (CASE 
                WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vSAID_FAT_CONTA_ORDEM UNION SELECT CODFISCAL FROM vSAID_REM_ENTREGA_FUTURA) THEN ROUND(NVL(T.CUSTOCONT_TV7, 0),4)
