...
FOR r IN (
 ...
 WITH 
  ...
  -----------------------GERENCIAL - CFOP DE ENTRADA
  vGER_ENT_COMPRA          AS (SELECT CODFISCAL FROM vENT_COMPRA UNION ALL SELECT CODFISCAL FROM vENT_COMPRA_CONSIGNADO UNION ALL SELECT CODFISCAL FROM vENT_COMPRA_TRIANGULAR),
  vGER_ENT_BONIFICADA      AS (SELECT CODFISCAL FROM vENT_BONIFICADA),
  vGER_ENT_DEVCLIENTE      AS (SELECT CODFISCAL FROM vENT_DEVOLUCAO),
	vGER_ENT_TRANSFERENCIA   AS (SELECT CODFISCAL FROM vENT_TRANSFERENCIA),
  
  -----------------------GERENCIAL - CFOP DE SAIDA
  vGER_SAID_VENDA          AS (SELECT CODFISCAL FROM vSAID_VENDA UNION ALL SELECT CODFISCAL FROM vSAID_REM_ENTREGA_FUTURA UNION ALL SELECT CODFISCAL FROM vSAID_FAT_CONTA_ORDEM),
  vGER_SAID_BONIFICADA     AS (SELECT CODFISCAL FROM vSAID_BONIFICADA),
  vGER_SAID_DEVFORNECEDOR  AS (SELECT CODFISCAL FROM vSAID_DEVOLUCAO),
  ...
  (SELECT M.NUMTRANSITEM AS NUMTRANSITEM_ENTTV9,
    ...
    WHERE M.CODFISCAL IN (SELECT CODFISCAL FROM vENT_DEMONSTRACAO)
     ...
     (SELECT M.NUMTRANSITEM,
             ...
             END) TIPOMOV,
             (CASE
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_ENT_COMPRA) THEN 'COMPRA'
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_ENT_BONIFICADA) THEN 'COMPRA BONIFICADA'
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_ENT_DEVCLIENTE) THEN 'DEVOLUCAO CLIENTE'
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_ENT_TRANSFERENCIA) THEN 'ENTRADA TRANSFERENCIA'
               ELSE 'ENTRADA DESCONSIDERAR'
             END) TIPOMOVGER,
             ...
             (CASE
               WHEN E.TIPODESCARGA IN ('6', '8', 'C', 'T') THEN ROUND(M.QTCONT * (M.PUNITCONT - NVL(M.VLFRETE, 0) - NVL(M.VLOUTRASDESP, 0) - NVL(M.VLIPI, 0) - NVL(M.ST, 0)), 2)
             ...
             END VLDESCONTO,
             (CASE
               WHEN LENGTH(TRIM(M.SITTRIBUT)) = 1 THEN LPAD(TRIM(M.SITTRIBUT), 2, '0')
               ELSE TRIM(M.SITTRIBUT)
             END) CST_ICMS,
             M.CODFISCAL CFOP,
             (CASE
             ...
             (CASE
               WHEN (NVL(M.GERAICMSLIVROFISCAL, 'S') = 'N' OR M.CODFILIAL IN ('5','6','9','10')) THEN 0
               ...
               ELSE ROUND(M.QTCONT * (NVL(M.BASEICMS, 0) * NVL(M.PERCICM, 0) / 100),2)
             END) VLICMS,
             0 VLICMSBENEFICIO,
             ...
             (CASE
               ...
               WHEN M.CODSITTRIBPISCOFINS IS NULL THEN '99'
               ELSE TO_CHAR(M.CODSITTRIBPISCOFINS)
             END) CST_PISCOFINS,
             ROUND(M.QTCONT * NVL(M.VLBASEPISCOFINS, 0), 2) VLBASEPISCOFINS,
             ROUND((NVL(M.PERPIS,0) / 100),4) PERCPIS,
             ROUND((NVL(M.PERCOFINS,0) / 100),4) PERCCOFINS,
     ...
     (SELECT M.NUMTRANSITEM,
             ...
             END) TIPOMOV,
             (CASE
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_SAID_VENDA) THEN 'VENDA'
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_SAID_BONIFICADA) THEN 'VENDA BONIFICADA'
               WHEN M.CODFISCAL IN (SELECT CODFISCAL FROM vGER_SAID_DEVFORNECEDOR) THEN 'DEVOLUCAO FORNECEDOR'
               ELSE 'SAIDA DESCONSIDERAR'
             END) TIPOMOVGER,
             ...
             ROUND((NVL(M.PERCICM, 0) / 100),4) PERCICMS,
             (CASE 
               WHEN M.CODFILIAL IN ('5','6','9','10') THEN 0 
               --ELSE ROUND(M.QTCONT * NVL(MC.VLICMS, (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) + NVL(MC.VLBASEOUTROS, 0)) * NVL(M.PERCICM, 0) / 100), 2)
               ELSE	ROUND(M.QTCONT * ((NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) + NVL(MC.VLBASEOUTROS, 0)) * NVL(M.PERCICM, 0) / 100),2)
             END) VLICMS,
             (CASE
   ...
   WHERE M.STATUS IN ('A', 'AB')
     ...
     WHERE 1 = 1 
       ...
       AND (S.DT_UPDATE IS NULL
        OR NVL(S.MOVIMENTO,'0') <> M.MOVIMENTO
        OR NVL(S.TIPOMOV,'0') <> M.TIPOMOV
        OR NVL(S.TIPOMOVGER,'0') <> M.TIPOMOVGER
