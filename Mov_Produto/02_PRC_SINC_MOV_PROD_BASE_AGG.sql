  ...
  FOR r IN (WITH MOV_AGG AS
              ...
              MOVIMENTACAO AS
               (SELECT L.CODEMPRESA,
                      ...
                      M.VALORCOFINS
                 FROM MOV_AGG M
                 LEFT JOIN BI_SINC_FILIAL L ON L.CODFILIAL =
                                               M.CODFILIAL
                 LEFT JOIN BI_SINC_VENDEDOR V ON V.CODUSUR =
                                                 M.CODUSUR
                 LEFT JOIN BI_SINC_FORNECEDOR F ON F.CODFORNEC =
                                                   M.CODFORNEC
                 LEFT JOIN BI_SINC_CLIENTE C ON C.CODCLI =
                                                M.CODCLI)
              
              SELECT M.*
                FROM MOVIMENTACAO M
                LEFT JOIN BI_SINC_MOV_PROD_BASE_AGG S ON S.NUMTRANSACAO =
                                                         M.NUMTRANSACAO
                                                         AND
                                                         S.NUMNOTA =
                                                         M.NUMNOTA
               WHERE 1 = 1
                     ...
                     S.CODFILIAL <> M.CODFILIAL OR
                     S.DATA <> M.DATA OR
                     S.MOVIMENTO <> M.MOVIMENTO OR
                     S.TIPOMOV <> M.TIPOMOV OR
                     NVL(S.TEMVENDAORIG,'0') <> M.TEMVENDAORIG OR
                     S.CODSUPERVISOR <> M.CODSUPERVISOR OR
                     S.CODGERENTE <> M.CODGERENTE OR
                     NVL(S.DTCANCEL,TO_DATE('01/01/1889','DD/MM/YYYY')) <> M.DTCANCEL OR
                     S.CODFORNEC <> M.CODFORNEC OR
                     S.FORNECEDOR <> M.FORNECEDOR OR
                     S.CLIENTE <> M.CLIENTE OR
                     S.CUSTOCONTABIL <> M.CUSTOCONTABIL OR
                     S.VALORCONTABIL <> M.VALORCONTABIL OR
                     S.VALORST <> M.VALORST OR
                     S.VALORSTGUIA <> M.VALORSTGUIA OR
                     S.VALORICMS <> M.VALORICMS OR
                     S.VALORICMSBENEFICIO <>
                     M.VALORICMSBENEFICIO OR
                     S.VALORICMSDIFAL <> M.VALORICMSDIFAL OR
                     S.VALORPIS <> M.VALORPIS OR
                     S.VALORCOFINS <> M.VALORCOFINS))
