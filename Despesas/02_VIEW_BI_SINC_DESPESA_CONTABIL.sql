CREATE OR REPLACE VIEW VIEW_BI_SINC_DESPESA_CONTABIL AS
    ...
    WITH LANCAMENTO AS
     (SELECT *
        FROM (SELECT L.RECNUM,
                     L.ANO_COMPETENCIA,
                     EXTRACT(MONTH FROM L.DTCOMPETENCIA) MES_COMPETENCIA,
                     L.CODFORNEC,
                     L.NUMNOTA,
                     ROW_NUMBER() OVER (PARTITION BY ANO_COMPETENCIA, EXTRACT(MONTH FROM L.DTCOMPETENCIA), CODFORNEC, NUMNOTA ORDER BY RECNUM) AS RN
    ...
    SELECT E.CODEMPRESA,
           ...
           NVL(E.VALOR, 0) VALOR,
           (CASE
             WHEN NVL(L.RECNUM, 0) = 0 THEN
              NVL(ROUND((E.VALOR * NVL(C.PERCRATEIO, 100) / 100), 4), NVL(E.VALOR, 0))
             ELSE
              FUNC_RATEAR_VALOR_POR_CC(E.VALOR, L.RECNUM, NVL(C.CODCC, '0'))
           END) VLRATEIO,
      ...
      LEFT JOIN LANCAMENTO L ON L.NUMNOTA = E.NUMNOTA
                            ...
                            AND L.MES_COMPETENCIA = EXTRACT(MONTH FROM E.DATA)
