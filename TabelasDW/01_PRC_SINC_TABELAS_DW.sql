...
NUMNOTA, DATA, CFOP, CODPROD, CODCLI, CODFORNEC, CODCOB, MOVIMENTO)';
  ...
  ON BI_SINC_CONTABILIDADE (DATA, CODCC, TIPOLANCAMENTO, DOCUMENTO, CODGERENCIAL, CODDRE,
...
CODFORNEC, ESPECIE, CFOP, CST_ICMS, CST_PISCOFINS )';
  ...
  ON BI_SINC_LANC_PAGAR_BASE (CODEMPRESA, CODFILIAL, ANO_COMPETENCIA, DTLANC,
  ...
  SELECT COUNT(*)
    INTO v_table_exists
    FROM user_tables
   WHERE table_name = 'BI_SINC_BANCO';
  IF v_table_exists = 0 THEN
  ...
  SELECT COUNT(*)
    INTO v_table_exists
    FROM user_tables
   WHERE table_name = 'BI_SINC_SALDO_BANCO';
  IF v_table_exists = 0 THEN
