...
BEGIN
  ...
  ON BI_SINC_MOV_PRODUTO (CODFILIAL, CODUSUR, NUMTRANSACAO, TEMVENDAORIG,
NUMNOTA, DATA, CFOP, CODPROD, CODCLI, CODFORNEC, CODCOB, MOVIMENTO)';
  END IF;

  ----BI_SINC_MOV_PROD_BASE_AGG
  SELECT COUNT(*)
    INTO v_table_exists
    FROM user_tables
   WHERE table_name = 'BI_SINC_MOV_PROD_BASE_AGG';
  IF v_table_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE TABLE BI_SINC_MOV_PROD_BASE_AGG
    (
      CODEMPRESA         VARCHAR2(2),
      CODFILIAL          VARCHAR2(2),
      DATA               DATE,
      ...
      DT_UPDATE          DATE,
      CONSTRAINT PK_MOV_PROD_BASE_AGG PRIMARY KEY (NUMTRANSACAO, NUMNOTA)
  ) ';
  END IF;
  ...
  SELECT COUNT(*)
    ...
    FROM user_indexes
   WHERE table_name = 'BI_SINC_MOV_PROD_BASE_AGG'
         AND index_name = 'MOV_PROD_BASE_AGG_IDX_01';
  IF v_index_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE INDEX MOV_PROD_BASE_AGG_IDX_01
  ON BI_SINC_MOV_PROD_BASE_AGG (TIPOMOV, CODFILIAL, MOVIMENTO, DATA,
TEMVENDAORIG, CODSUPERVISOR, CODGERENTE, CODFORNEC, DTCANCEL)';
  END IF;
  ...
  SELECT COUNT(*)
    INTO v_table_exists
    FROM user_tables
   WHERE table_name = 'BI_SINC_PLANO_CONTAS_ESTILO';
  IF v_table_exists = 0 THEN
  ...
  (
     ...
     CONTAN4       VARCHAR2(60),
     DT_UPDATE     DATE,
     CONSTRAINT PK_PLANO_CONTAS_ESTILO PRIMARY KEY (ID)
  ) ';
  END IF;
  ...
  ----BI_SINC_MOV_CONTABIL
  SELECT COUNT(*)
    INTO v_table_exists
    FROM user_tables
   WHERE table_name = 'BI_SINC_MOV_CONTABIL';
  IF v_table_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE TABLE BI_SINC_MOV_CONTABIL
  (
     CODLANC         VARCHAR2(3),
     CODEMPRESA      VARCHAR2(2),
     DATA            DATE,
     ...
     DTCANCEL        DATE,
     DT_UPDATE      DATE,
     CONSTRAINT PK_MOV_CONTABIL PRIMARY KEY (CODLANC, IDENTIFICADOR, CODCC_DEBITO, CODCC_CREDITO)
  )';
  END IF;
  
    ----INDICES BI_SINC_MOV_CONTABIL
  SELECT COUNT(*)
    INTO v_index_exists
    FROM user_indexes
   WHERE table_name = 'BI_SINC_MOV_CONTABIL'
         AND index_name = 'MOV_CONTABIL_IDX_01';
  IF v_index_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE INDEX MOV_CONTABIL_IDX_01
  ON BI_SINC_MOV_CONTABIL (DATA, TIPOLANCAMENTO, DOCUMENTO, CONTADEBITO,
CONTACREDITO, ATIVIDADE, ORIGEM, ENVIAR_CONTABIL)';
  END IF;
  ...
  (
     ...
     ENVIAR_CONTABIL VARCHAR2(1),
     DT_UPDATE      DATE,
     CONSTRAINT PK_CONTABILIDADE PRIMARY KEY (CODLANC, IDENTIFICADOR, OPERACAO, CODCC)
  )';
  END IF;
  ...
  ON BI_SINC_CONTABILIDADE (DATA, TIPOLANCAMENTO, DOCUMENTO, CODGERENCIAL, CODDRE,
  CODCONTABIL, ATIVIDADE, ORIGEM, ENVIAR_CONTABIL)';
  END IF;
...
CODFORNEC, ESPECIE, CFOP, CST_ICMS, CST_PISCOFINS )';
  ...
  ----BI_SINC_DESPESA_FISCAL_BASE
  SELECT COUNT(*)
    INTO v_table_exists
    FROM user_tables
   WHERE table_name = 'BI_SINC_DESPESA_FISCAL_BASE';
  IF v_table_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE TABLE BI_SINC_DESPESA_FISCAL_BASE
  (
     CODEMPRESA      VARCHAR2(2),
     CODFILIAL       VARCHAR2(2),
     DATA            DATE,
     ...
     CODCC           VARCHAR2(3),
     PERCRATEIO      NUMBER(12, 2),
     DT_UPDATE       DATE,
     CONSTRAINT PK_DESPESA_FISCAL_BASE PRIMARY KEY (NUMTRANSENT, NUMNOTA, CODCC)
  )';
  END IF;
  
    ----INDICES BI_SINC_DESPESA_FISCAL_BASE
  SELECT COUNT(*)
    INTO v_index_exists
    FROM user_indexes
   WHERE table_name = 'BI_SINC_DESPESA_FISCAL_BASE'
         AND index_name = 'DESPESA_FISCAL_BASE_IDX_01';
  IF v_index_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE INDEX DESPESA_FISCAL_BASE_IDX_01
  ON BI_SINC_DESPESA_FISCAL_BASE (CODEMPRESA, CODFILIAL, DATA, NUMNOTA, CODCONTA,
CODFORNEC, ESPECIE, CFOP, RECNUM )';
  END IF;
  ...
  ON BI_SINC_LANC_PAGAR_BASE (CODEMPRESA, CODFILIAL, ANO_COMPETENCIA, 
  ADIANTAMENTO, DTCOMPETENCIA, DTVENCIMENTO, TIPO, CODCC, CODCONTA, CODFORNEC, 
  NUMNOTA, NUMTRANS, DTPAGAMENTO, DTCOMPENSACAO, CODEMPRESABANCO, CONTABANCO)';
  END IF;
    ...
    FROM user_indexes
   WHERE table_name = 'BI_SINC_LANC_PAGAR_BASE'
         AND index_name = 'LANC_PAGAR_BASE_IDX_02';
  IF v_index_exists = 0 THEN
    EXECUTE IMMEDIATE 'CREATE INDEX LANC_PAGAR_BASE_IDX_02
  ON BI_SINC_LANC_PAGAR_BASE (NUMNOTA, CODFORNEC, ANO_COMPETENCIA)';
  END IF;
