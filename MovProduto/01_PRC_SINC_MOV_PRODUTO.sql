  ...
  INSERT INTO TEMP_PCMOV
    ...
    WITH ENTRADAS AS
     (SELECT M.NUMTRANSITEM,
             ...
             M.QTCONT QT,
             (CASE
               WHEN E.TIPODESCARGA IN ('6', '8', 'C', 'T') THEN NVL(M.CUSTOFINEST, 0)
               ELSE NVL(M.CUSTOFIN, 0)
             END) CUSTOFINANCEIRO,
             NVL(M.CUSTOREP, 0) CUSTOREPOSICAO,
             NVL(M.CUSTOCONT, 0) CUSTOCONTABIL,
             (CASE
               WHEN (E.TIPODESCARGA IN ('6', '8', 'C', 'T')) THEN ROUND(M.QTCONT * (M.PUNITCONT + NVL(M.VLFRETE, 0) + NVL(M.VLOUTROS, 0)), 2)
               WHEN E.TIPODESCARGA IN ('N', 'F', 'I') THEN (M.QTCONT * M.PUNITCONT)
               ELSE M.QTCONT * (M.PUNITCONT + NVL(M.VLIPI, 0) + NVL(M.ST, 0) + NVL(M.VLFRETE, 0) + NVL(M.VLOUTRASDESP, 0) - NVL(M.VLDESCONTO, 0) - NVL(M.VLSUFRAMA, 0))
             END) VLCONTABIL,
             M.PUNITCONT PUNIT,
             M.PTABELA,
             (CASE
               ...
               WHEN E.TIPODESCARGA IN ('N', 'F', 'I') THEN M.QTCONT * (M.PUNITCONT + NVL(M.VLDESCONTO, 0) - NVL(M.VLFRETE, 0) - NVL(M.VLOUTRASDESP, 0) - NVL(M.ST, 0))
             ...
             (M.PERPIS / 100) PERCPIS,
             (M.PERCOFINS / 100) PERCCOFINS,
             ...
             (ROUND(NVL(M.QTCONT, 0) * NVL(MC.VLICMSPARTREM, 0), 2) +
             ROUND(NVL(M.QTCONT, 0) * NVL(MC.VLICMSPARTDEST, 0), 2) +
             ROUND(NVL(M.QTCONT, 0) * NVL(MC.VLFCPPART, 0), 2)) VLICMSDIFAL
        FROM PCNFENT E
    ...
    SAIDAS AS
     (SELECT M.NUMTRANSITEM,
             ...
             M.CODPROD,
             M.QTCONT QT,
             NVL(M.CUSTOFINEST, M.CUSTOFIN) CUSTOFINANCEIRO,
             NVL(M.CUSTOREP, 0) CUSTOREPOSICAO,
             NVL(M.CUSTOCONT, 0) CUSTOCONTABIL,
             (CASE
             ...
             M.PTABELA,
             (CASE
               WHEN (NVL(M.CODOPER, 'X') <> 'SDF') THEN ROUND(M.QTCONT * (M.PUNITCONT + NVL(M.VLDESCONTO, 0) - NVL(M.ST, 0) - NVL(M.VLIPI, 0)), 2)
               ELSE ROUND(M.QTCONT * (M.PUNITCONT + NVL(M.VLDESCONTO, 0) - NVL(M.ST, 0) - NVL(M.VLIPI, 0) - NVL(M.VLFRETE, 0)), 2)
             END) VLPRODUTO,
             ...
             (CASE
               WHEN (M.DTMOV >= TO_DATE('01/09/2023', 'DD/MM/YYYY') AND M.CODFILIAL = '11' AND M.PERCICM > 0) THEN 0.011385 * (M.QTCONT * (NVL(M.BASEICMS, 0) + NVL(MC.VLBASEFRETE, 0) + NVL(MC.VLBASEOUTROS, 0)))
               ELSE 0
             END) VLICMSBENEFICIO,
