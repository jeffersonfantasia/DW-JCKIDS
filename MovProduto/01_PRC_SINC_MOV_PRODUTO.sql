CREATE OR REPLACE PROCEDURE PRC_SINC_MOV_PRODUTO AS
   ...
   vDATA_MOV_INCREMENTAL DATE := TRUNC(SYSDATE) - 30;
  ...
  (SELECT M.NUMTRANSITEM AS NUMTRANSITEM_ENTTV9,
    ...
    WHERE M.CODFISCAL IN (1913, 2913)
     ...
     (SELECT M.NUMTRANSITEM,
             ...
             ROUND(NVL(M.CUSTOREP, 0),4) CUSTOREPOSICAO,
             (CASE 
                WHEN M.CODFISCAL IN (5923, 6923, 5117, 6117) THEN ROUND(NVL(T.CUSTOCONT_TV7, 0),4)
             ...
             END) VLICMS,
             (CASE
               WHEN (M.DTMOV >= TO_DATE('01/09/2023', 'DD/MM/YYYY') AND M.CODFILIAL = '11' AND M.PERCICM > 0 AND NVL(M.GERAICMSLIVROFISCAL, 'S') = 'S') 
